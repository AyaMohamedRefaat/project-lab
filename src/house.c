...
void houseInit(void)
{
   ...
   for (uint8_t i = 0; i < ROOM_COUNT; i++)
    {
        uint8_t j = 0;
        while (NAMES[i][j] != '\0' && j < (NAME_LEN - 1))
        {
...
{
    uint32_t intermediate = (uint32_t)adc * 500U;
    uint32_t result = intermediate / 1024U;
...
uint8_t applyRules(Room_t *r)
{
    ...
    uint16_t room_temp = tempC(r->adc);
...
uint32_t sumAdc(const Room_t *rooms, uint8_t n)
{
    ...
    uint32_t current_adc = (uint32_t)rooms[n - 1U].adc;
    uint32_t rest_sum = sumAdc(rooms, n - 1U);
    uint32_t total = current_adc + rest_sum;
